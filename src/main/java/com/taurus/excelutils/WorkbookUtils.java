...
//		cell.setEncoding(HSSFWorkbook.ENCODING_UTF_16);
	...
	public static void copyRow(Sheet sheet, int from, int to, int count) {
		...
		for (int i = 0; i < sheet.getNumMergedRegions(); i++) {
			...
			if (r.getFirstColumn() >= from && r.getLastRow() < from + count) {
				int firstRow = r.getFirstRow() + to - from;
				int lastRow = r.getLastRow() + to - from;
				int firstCol = r.getFirstColumn();
				int lastCol =r.getLastColumn();
				shiftedRegions.add(new CellRangeAddress(firstRow,lastRow,firstCol,lastCol));
			}
		}
	...
	public static void shiftCell(Sheet sheet, Row row, Cell beginCell, int shift, int rowCount) {
		...
		for (int rownum = fromRow; rownum <= toRow; rownum++) {
			...
			for (int cellpos = lastCellNum; cellpos >= beginCell.getRowIndex(); cellpos--) {
